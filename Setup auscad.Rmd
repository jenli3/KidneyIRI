...
library(devtools)
packagesload <- c()
packagesload <- c("tidyverse", "dplyr", "readxl", "ggpubr", "graphics", "eeptools",
                  "Gmisc", "knitr", "table1", "MatchIt", "htmlTable", "magrittr", 
                  "survminer", "survival", "lattice", "rstatix", "viridis", "pROC",  
                  "reshape2", "rcompanion", "edgeR", "limma", "Glimma", "gridExtra",
                  "AnnotationDbi", "GOexpress", "Matrix", "org.Hs.eg.db", "org.Mm.eg.db", 
                  "ggfortify",  "RColorBrewer", "mixOmics", "VennDiagram", "monocle", 
                  "Rtsne", "gplots", "ggplot2", "NMF", "EnhancedVolcano", "calibrate", 
                  "pheatmap", "vsn", "Seurat", "SeuratObject", "FSA", "pathfindR", 
                  "PCAtools", "rrcov", "pcaExplorer", "usethis", "devtools", "ComICS", 
                  "pathview", "rjson", "cowplot", "grid", "readbitmap", "rhdf5", 
                  "hdf5r",  "data.table", "tibble", "clusterProfiler", "ReactomePA", 
                  "pathview",  "enrichplot", "DOSE", "superheat", "stringr",  
                  "Rcpp", "NMF",  "kableExtra", "imager", "spData", "spdep", 
                  "topGO", "biomaRt", "STdeconvolve", "STutility", "SpatialCPie", 
                  "clustree", "pander",  "pagoda2", "harmony", "SingleCellExperiment", 
                  "SCINA", "shiny", "xtable","S4Vectors", "MASS", "singleCellHaystack", 
                  "NMF", "gt", "igraph", "GOplot", "ggnewscale", "ggvenn", "GEOquery", 
                  "forcats", "stringr", "ggrepel", "readr", "tidyr", "XML", "harmony", 
                  "MLeval", "GGally", "gtsummary", "finalfit", "caret", "shiny", 
                  "SingleR", "scmap", "celldex", "SingleCellExperiment", "SCINA", 
                  "ggVennDiagram", "directPA", "Nebulosa", "nnet", "broom",
                  "MuSiC", "Capybara", "granulator", "scMappR","patchwork")
...
sort.normalise = function(sample, counts, spec.group){
  ...
  groups.sample1 = groups.sample1 %>% gather(colnames(groups.sample1[,-ncol(groups.sample1)]), 
  ...
  keep1 = filterByExpr(dge.auscad, group=dge.auscad$samples$group) # remove lowly expressed
  dge.auscad = dge.auscad[keep1, keep.lib.sizes = FALSE]
  dge.auscad = calcNormFactors(dge.auscad, method = "TMM") # Normalise
...
clean.data.R
```{r, echo=TRUE, results = FALSE, warning = FALSE, eval = FALSE}
# DONT run this section - my code for data cleaning and aggregation
# set.seed(42)
# Section 1: clin annotation
# Load clinical metadata
# setwd("~/Documents/Project data - R files/auscad/")
auscad = read_xlsx("data/base/JenAuscadDB2022.xlsx")
auscad$age = as.numeric(difftime(Sys.Date(), auscad$dob,  units = "weeks"))/52.25

# DGF criteria
dgf_filter = c()
dgf_filter = auscad[!is.na(auscad$graft_age_months),]
dgf_filter = dgf_filter%>% mutate(dgf_hd_criteria1 = 
                                    factor(dgf_filter$dgf_hd_criteria, levels=c(0,1), 
                                           labels=c("Control", "DGF.HD")))
dgf_filter = dgf_filter%>% mutate(dgf_cr_criteria1 = 
                                    factor(dgf_filter$dgf_cr_criteria, levels=c(0,1), 
                                           labels=c("Control", "DGF.Cr")))
dgf_filter = dgf_filter%>% mutate(dgf_combined1 = 
                                    factor(dgf_filter$dgf_combined, levels=c(0,1), 
                                           labels=c("Control", "DGF")))
dgf_filter = dgf_filter %>%
  mutate(dgf_severity = case_when((dgf_combined == 0 & dgf_hd_criteria == 0) ~ "Control",
                                  (dgf_combined == 1 & dgf_hd_criteria == 0) ~ "Cr drop < 20%",
                                  (dgf_combined == 1 & dgf_hd_criteria == 1 & dgf_hd_days <= 7) ~ "HD <= 7 days",
                                  (dgf_combined == 1 & dgf_hd_criteria == 1 & dgf_hd_days > 7) ~ "HD > 7 days"))
dgf_filter$dgf_severity[is.na(dgf_filter$dgf_severity)] = "Control"

dgf_filter$dgf_define = ifelse(dgf_filter$dgf_hd_criteria == 1, "HD", "Control")
dgf_filter$dgf_define = ifelse(dgf_filter$dgf_cr_criteria == 1, "Cr", dgf_filter$dgf_define)
dgf_filter$dgf_define = ifelse(is.na(dgf_filter$dgf_define), "Control", dgf_filter$dgf_define)

# diff DGF severity for d1 and d2
dgf_filter$cr.drop.d1 = -100*(as.numeric(dgf_filter$cr_d1) -
                                as.numeric(dgf_filter$cr_0hr))/as.numeric(dgf_filter$cr_0hr)
dgf_filter$cr.drop.d2 = -100*(as.numeric(dgf_filter$cr_d2) -
                                as.numeric(dgf_filter$cr_0hr))/as.numeric(dgf_filter$cr_0hr)
dgf_filter = dgf_filter %>% mutate(d1.drop =  
                                     case_when(cr.drop.d1 >=20 ~ "control",
                                               cr.drop.d1 < 10 ~ "drop.10pc",
                                               cr.drop.d1 > 10 & cr.drop.d1 < 20 ~ "drop.20pc"))

dgf_filter$d1.drop = ifelse(dgf_filter$dgf_define == "HD", "HD", dgf_filter$d1.drop)
dgf_filter$d1.drop = ifelse(is.na(dgf_filter$d1.drop), "control", dgf_filter$d1.drop)

dgf_filter = dgf_filter %>% mutate(d2.drop =  
                                     case_when(cr.drop.d2 >=20 ~ "control",
                                               cr.drop.d2 < 10 ~ "drop.10pc",
                                               cr.drop.d2 > 10 & cr.drop.d1 < 20 ~ "drop.20pc"))
dgf_filter$d2.drop = ifelse(dgf_filter$dgf_define == "HD", "HD", dgf_filter$d2.drop)
dgf_filter$d2.drop = ifelse(is.na(dgf_filter$d2.drop), "control", dgf_filter$d2.drop)


# Donor criteria
dgf_filter = dgf_filter %>% mutate(donor_criteria =
                                     case_when((donor_criteria_base ==0 & donor_criteria_ECD == 0) ~ "DBD",
                                               (donor_criteria_base ==0 & donor_criteria_ECD == 1) ~ "DBD + ECD",
                                               (donor_criteria_base ==1 & donor_criteria_ECD == 0) ~ "DCD",
                                               (donor_criteria_base ==1 & donor_criteria_ECD == 1) ~ "DCD + ECD",
                                               (donor_criteria_base ==3 & donor_criteria_ECD == 0) ~ "LKD"))
...
# --- CADI scores --- sum of i, mm, ci, ct, cv, gs (% affected)
dgf_filter$cadi_1m=round(dgf_filter$i_1m + dgf_filter$mm_1m + dgf_filter$ci_1m +
                           dgf_filter$ct_1m + dgf_filter$cv_1m +
                           (dgf_filter$sclerosed_1m/dgf_filter$gloms_1m), 2)

dgf_filter$cadi_3m=round(dgf_filter$i_3m + dgf_filter$mm_3m + dgf_filter$ci_3m +
                           dgf_filter$ct_3m + dgf_filter$cv_3m +
                           (dgf_filter$sclerosed_3m/dgf_filter$gloms_3m), 2)

dgf_filter$cadi_12m=round(dgf_filter$i_12m + dgf_filter$mm_12m +
                            dgf_filter$ci_12m +dgf_filter$ct_12m + dgf_filter$cv_12m +
                            (dgf_filter$sclerosed_12m/dgf_filter$gloms_12m), 2)

cadi_combined=cbind(dgf_filter$cadi_1m, dgf_filter$cadi_3m, dgf_filter$cadi_12m)


dgf_filter$cadi_1m1<-factor(dgf_filter$cadi_1m)
dgf_filter$cadi_3m1<-factor(dgf_filter$cadi_3m)
dgf_filter$cadi_12m1<-factor(dgf_filter$cadi_12m)

#--- make new variable for event type combined: nil = 0, death=1, deathFG = 2, lossFU = 3, kidney loss = 4, panc loss = 5
dgf_filter = dgf_filter %>% mutate(event_all =
                                     case_when((event_any ==0 & death == 0 & death_functioninggraft == 0
                                                & censor_followup == 0 & graft_loss ==0 ) ~ "Nil",
                                               (event_any ==1 & death == 1) ~ "death",
                                               (event_any ==1 & death == 1 & death_functioninggraft == 1
                                                & censor_followup == 0 & graft_loss ==0) ~ "death_fg",
                                               (event_any ==1 & death == 0 & death_functioninggraft == 0
                                                & censor_followup == 1 & graft_loss ==0) ~ "loss_fu",
                                               (event_any ==1 & death == 0 & death_functioninggraft == 0
                                                & censor_followup == 0 & graft_loss ==1) ~ "graft_loss_only"))

# Death, graft loss as main events (event.main)
dgf_filter$event.main = as.numeric(ifelse(dgf_filter$event_all == "Nil"|
                                            dgf_filter$event_all == "loss_fu", 0, 1))
dgf_filter$event.main = ifelse(is.na(dgf_filter$event.main), 0, dgf_filter$event.main)

dgf_filter$death.cs.graftloss = ifelse(dgf_filter$event_all == "death" |
                                         dgf_filter$event_all == "graft_loss_only", 
                                       "yes", "no")


dgf_filter$age = round(as.numeric(difftime(Sys.Date(), dgf_filter$dob, 
                                           units = "weeks"))/52.25, digits = 1)
dgf_filter$temp.months = round(as.numeric(difftime(dgf_filter$review_date_J_S,
                                                   dgf_filter$date_tx, 
                                                   units = "weeks"))/4, digits = 1)
dgf_filter$temp.event = round(as.numeric(difftime(dgf_filter$date_loss, 
                                                  dgf_filter$date_tx, 
                                                  units = "weeks"))/4, digits = 1)
dgf_filter$time.in.study = dgf_filter$temp.months
dgf_filter$time.in.study = as.numeric(ifelse(!is.na(dgf_filter$temp.event),
                                  dgf_filter$temp.event, 
                                  dgf_filter$time.in.study))

dgf_filter$dgf_hdsessions = as.numeric(dgf_filter$dgf_hdsessions)
dgf_filter$dgf_hd_days = as.numeric(dgf_filter$dgf_hd_days)

# other events
dgf_filter$bpar_any = ifelse((dgf_filter$bpar_0_1m ==1 | dgf_filter$bpar_1m == 1 |
                                dgf_filter$bpar_3m == 1 | dgf_filter$bpar_3_12m == 1 |
                                dgf_filter$bpar_12m == 1 | dgf_filter$bpar_post12m == 1),  1, 0)
dgf_filter$bpar_any[is.na(dgf_filter$bpar_any)] = 0
dgf_filter$bpar_any1 = factor(dgf_filter$bpar_any,
                              level=c(0,1), label=c("No rejection", "BPAR any time"))

dgf_filter = dgf_filter %>% mutate(subclin_any =  ifelse((Subclin_0_1m ==1 | subclin_1m == 1 |
                                                            subclin_3m == 1 | subclin_3_12m == 1 |
                                                            subclin_12m == 1 | subclin_post12m == 1),  1, 0))
dgf_filter$subclin_any[is.na(dgf_filter$subclin_any)] = 0
dgf_filter$subclin_any1 = factor(dgf_filter$subclin_any,
                                 level=c(0,1), label=c("No rejection", "Subclin Rejection any time"))

dgf_filter = dgf_filter %>%  mutate(bkvan_any =
                                      ifelse((bkvan_0_1m ==1 | bk_van_1m == 1 |
                                                bk_van_1_3m == 1 |   bk_van_3m == 1 |
                                                bk_van_3_12m == 1 | bk_van_12m == 1 |
                                                bk_van_post12m==1),  1, 0))
dgf_filter$bkvan_any[is.na(dgf_filter$bkvan_any)] = 0
dgf_filter$bkvan_any1 = factor(dgf_filter$bkvan_any,
                               level=c(0,1), label=c("No BKVAN", "BKVAN any time"))

dgf_filter = dgf_filter %>% mutate(mpred_any =
                                     ifelse((mpred_0_1m ==1 | mpred_1m == 1 |
                                               mpred_1_3m == 1 | mpred_3m == 1 |
                                               mpred_3_12m == 1 | mpred_12m == 1),  1, 0))
dgf_filter$mpred_any[is.na(dgf_filter$mpred_any)] = 0
dgf_filter$mpred_any1 =factor(dgf_filter$mpred_any,
                              level=c(0,1), label=c("No Mpred", "Methylprednisolone any time"))

dgf_filter = dgf_filter %>%  mutate(steroidres_any =
                                      ifelse((steroid_res_0_1m ==1 | steroid_res_1m == 1 |
                                                steroid_res_1_3m == 1 |steroid_res_3m == 1 |
                                                steroid_res_3_12m == 1 | steroid_res_12m == 1),  1, 0))
dgf_filter$steroidres_any[is.na(dgf_filter$steroidres_any)] = 0
dgf_filter$steroidres_any1 = factor(dgf_filter$steroidres_any,
                                    level=c(0,1), label=c("No additional IS", "Additional IS"))

dgf_filter = dgf_filter %>%  mutate(bparupto03m =
                                      ifelse((bpar_0_1m ==1 | bpar_1m == 1 |
                                                bpar_1m == 1 |bpar_3_12m == 1),  1, 0))
dgf_filter$bparupto03m[is.na(dgf_filter$bparupto03m)] = 0

# Recipient variables
dgf_filter$gender = factor(dgf_filter$gender,
                           levels=c(0,1), labels=c("Male", "Female"))
dgf_filter$ethnicity = factor(dgf_filter$ethnicity,  levels=c(0,1,2,3),
                              labels=c("Caucasian", "Asian","Islander","Indigenous"))
dgf_filter$htn_pre  =  factor(dgf_filter$htn_pre,
                              levels=c(0,1), labels=c("Nil", "HTN pre-Tx"))
dgf_filter$dm_pre = factor(dgf_filter$dm_pre,
                           levels=c(0,1), labels=c("Nil", "DM pre-Tx"))
dgf_filter$ihd_pre = factor(dgf_filter$ihd_pre,
                            levels=c(0,1), labels=c("Nil", "IHD pre-Tx"))
dgf_filter$chol_pre = factor(dgf_filter$chol_pre,
                             levels=c(0,1), labels=c("Nil", "CHOL pre-Tx"))
dgf_filter$ondialysis_pretx = factor(dgf_filter$ondialysis_pretx,
                                     levels=c(0,1),
                                     labels=c("Pre-emptive Tx", "Pre-Tx Dialysis"))

dgf_filter$organ_type = factor(dgf_filter$organ_type,
                               levels=c(0,1,2),
                               labels=c("Kidney Transplant", "Living Kidney Transplant", "SPK"))
dgf_filter$graft_number = factor(dgf_filter$graft_number,
                                 levels=c(1,2), labels=c("1st Graft", "Regraft"))
dgf_filter$tx_age = as.numeric(dgf_filter$tx_age)
dgf_filter$HLA_mm = as.numeric(dgf_filter$HLA_mm)
dgf_filter$dsa1_pre = factor(dgf_filter$dsa1_pre, c(0,1),
                             labels=c("No Class I DSA", "Pre-Tx Class I DSA"))
dgf_filter$dsa2_pre = factor(dgf_filter$dsa2_pre, c(0,1),
                             labels=c("No Class II DSA", "Pre-Tx Class II DSA"))
dgf_filter$diabetickidney = factor(ifelse(dgf_filter$renal_dx==2,1,0))

# Donor variables
dgf_filter$donor_criteria_base = factor(dgf_filter$donor_criteria_base,
                                        levels=c(0,1,3),
                                        labels=c("DBD", "DCD","Living"))
dgf_filter$donor_gender = factor(dgf_filter$donor_gender,
                                 levels=c(0,1),
                                 labels=c("Male", "Female"))
dgf_filter$donor_inotropes = factor(dgf_filter$donor_ionotropes,
                                    levels=c(0,1),
                                    labels=c("No Inotropes", "Inotropic support"))
dgf_filter$donor_age = as.numeric(dgf_filter$donor_age)
dgf_filter$donor_admission_urea = as.numeric(dgf_filter$donor_admission_urea)
dgf_filter$donor_admission_cr = as.numeric(dgf_filter$donor_admission_cr)
dgf_filter$donor_terminal_urea = as.numeric(dgf_filter$donor_terminal_urea)
dgf_filter$donor_terminal_cr = as.numeric(dgf_filter$donor_terminal_cr)
dgf_filter$donor_intubated_days = as.numeric(dgf_filter$donor_intubated_days)
dgf_filter$anastamosis_time_mins = as.numeric(dgf_filter$anastamosis_time_mins)

### outcome variables
dgf_filter$death = factor(dgf_filter$death, levels=c(0,1), labels=c("Alive", "Deceased"))
dgf_filter$death_functioninggraft = factor(dgf_filter$death_functioninggraft,
                                           levels=c(0,1),
                                           labels=c("NA",
                                                    "Death with functioning graft"))
dgf_filter$graft_loss = factor(dgf_filter$graft_loss,
                               levels=c(0,1,2),
                               labels=c("Functioning graft",
                                        "Kidney Loss","Pancreas Loss"))
dgf_filter$censor_followup = factor(dgf_filter$censor_followup,
                                    levels=c(0,1,2),
                                    labels=c("current","loss to followup >=5yrs no data",
                                             "declined further"))
dgf_filter$graft_loss_kidneyonly = factor(ifelse(dgf_filter$graft_loss==1,1,0))
dgf_filter$nodat = factor(dgf_filter$nodat,
                          levels=c(0,1),
                          labels=c("No NODAT", "NODAT"))
dgf_filter$nodat_rx = factor(dgf_filter$nodat_rx,
                             levels=c(0,1,2),
                             labels=c("Nil", "Oral Hypoglycemics", "Insulin"))
dgf_filter$wound_infection_01m = factor(dgf_filter$wound_infection_01m,
                                        levels=c(0,1),
                                        labels=c("Nil", "Wound infection"))
dgf_filter$surgical_collection_01m = factor(dgf_filter$surgical_collection_01m,
                                            levels=c(0,1),
                                            labels=c("Nil", "Post op collection"))
dgf_filter$return_OT_01m = factor(dgf_filter$return_OT_01m,
                                  levels=c(0,1),
                                  labels=c("Nil", "Return to OT (<1m)"))
dgf_filter$anticoag_01m = factor(dgf_filter$anticoag_01m,
                                 levels=c(0,1),
                                 labels=c("Nil", "Therapeutic anticoagulation (<1m)"))
dgf_filter$transfusion_01m = factor(dgf_filter$transfusion_01m,
                                    levels=c(0,1),
                                    labels=c("Nil", "Transfusion (<1m)"))
dgf_filter$transfuse_1yr = factor(dgf_filter$transfuse_1yr,
                                  levels=c(0,1),
                                  labels=c("Nil", "Transfusion (<12m)"))
dgf_filter$readmission_1yr = factor(dgf_filter$readmission_1yr,
                                    levels=c(0,1),
                                    labels=c("Nil", "Readmission (<12m)"))

dgf_filter$cr_pre  =  as.numeric(dgf_filter$cr_pre)
dgf_filter$cr_0hr  = as.numeric(dgf_filter$cr_0hr)
dgf_filter$cr_d1 = as.numeric(dgf_filter$cr_d1)
dgf_filter$cr_d2 = as.numeric(dgf_filter$cr_d2)
dgf_filter$tac_d2 = as.numeric(dgf_filter$tac_d2)
dgf_filter$tac_d7 = as.numeric(dgf_filter$tac_d7)
dgf_filter$tac_d14 = as.numeric(dgf_filter$tac_d14)
dgf_filter$tac_d21 = as.numeric(dgf_filter$tac_d21)
dgf_filter$tac_1m  = as.numeric(dgf_filter$tac_1m)
dgf_filter$tac_3m  =  as.numeric(dgf_filter$tac_3m)
dgf_filter$tac_12m =  as.numeric(dgf_filter$tac_12m)
dgf_filter$cr_1m  =  as.numeric(dgf_filter$cr_1m)
dgf_filter$egfr_1m  = as.numeric(dgf_filter$egfr_1m)
dgf_filter$uacr_1m =  as.numeric(dgf_filter$uacr_1m)
dgf_filter$cr_3m  =  as.numeric(dgf_filter$cr_3m)
dgf_filter$egfr_3m  = as.numeric(dgf_filter$egfr_3m)
dgf_filter$uacr_3m =  as.numeric(dgf_filter$uacr_3m)
dgf_filter$mgfr_3m  =  as.numeric(dgf_filter$mgfr_3m)
dgf_filter$cr_12m  =  as.numeric(dgf_filter$cr_12m)
dgf_filter$egfr_12m  =  as.numeric(dgf_filter$egfr_12m)
dgf_filter$uacr_12m  =  as.numeric(dgf_filter$uacr_12m)
dgf_filter$mgfr_12m  =  as.numeric(dgf_filter$mgfr_12m)
dgf_filter$cr_24m  =  as.numeric(dgf_filter$cr_24m)
dgf_filter$egfr_24m  =  as.numeric(dgf_filter$egfr_24m)
dgf_filter$cr_36m  =  as.numeric(dgf_filter$cr_36m)
dgf_filter$egfr_36m  =  as.numeric(dgf_filter$egfr_36m)
dgf_filter$cr_48m  =  as.numeric(dgf_filter$cr_48m)
dgf_filter$egfr_48m  =  as.numeric(dgf_filter$egfr_48m)
dgf_filter$cr_60m  =  as.numeric(dgf_filter$cr_60m)
dgf_filter$egfr_60m  =  as.numeric(dgf_filter$egfr_60m)

### biopsy factors and labels
dgf_filter$bpar_0_1m = factor(dgf_filter$bpar_0_1m,
                              levels=c(0,1),
                              labels=c("No BPAR", "BPAR (0-1m)"))
dgf_filter$bpar_1m = factor(dgf_filter$bpar_1m,
                            levels=c(0,1),
                            labels=c("No BPAR", "BPAR (1m)"))
dgf_filter$bpar_1_3m = factor(dgf_filter$bpar_1_3m,
                              levels=c(0,1),
                              labels=c("No BPAR", "BPAR (1-3m)"))
dgf_filter$bpar_3m = factor(dgf_filter$bpar_3m,
                            levels=c(0,1),
                            labels=c("No BPAR", "BPAR (3m)"))
dgf_filter$bpar_3_12m = factor(dgf_filter$bpar_3_12m,
                               levels=c(0,1),
                               labels=c("No BPAR", "BPAR (3-12m)"))
dgf_filter$bpar_12m = factor(dgf_filter$bpar_12m,
                             levels=c(0,1),
                             labels=c("No BPAR", "BPAR (12m)"))
dgf_filter$bpar_post12m = factor(dgf_filter$bpar_post12m,
                                 levels=c(0,1),
                                 labels=c("No BPAR", "BPAR (>12m)"))
dgf_filter$Subclin_0_1m = factor(dgf_filter$Subclin_0_1m,
                                 levels=c(0,1),
                                 labels=c("No Subclin", "Subclin (0-1m)"))
dgf_filter$subclin_1m = factor(dgf_filter$subclin_1m,
                               levels=c(0,1),
                               labels=c("No Subclin", "Subclin (1m)"))
dgf_filter$subclin_1_3m = factor(dgf_filter$subclin_1_3m,
                                 levels=c(0,1),
                                 labels=c("No Subclin", "Subclin (1-3m)"))
dgf_filter$subclin_3m = factor(dgf_filter$subclin_3m,
                               levels=c(0,1),
                               labels=c("No Subclin", "Subclin (3m)"))
dgf_filter$subclin_3_12m = -factor(dgf_filter$subclin_3_12m,
                                   levels=c(0,1),
                                   labels=c("No Subclin", "Subclin (3-12m)"))
dgf_filter$subclin_12m = factor(dgf_filter$subclin_12m,
                                levels=c(0,1),
                                labels=c("No Subclin", "Subclin (12m)"))
dgf_filter$subclin_post12m = factor(dgf_filter$subclin_post12m,
                                    levels=c(0,1),
                                    labels=c("No Subclin", "Subclin (>12m)"))
dgf_filter$mpred_0_1m = factor(dgf_filter$mpred_0_1m,
                               levels=c(0,1),
                               labels=c("No mpred", "mpred (0-1m)"))
dgf_filter$mpred_1m = factor(dgf_filter$mpred_1m,
                             levels=c(0,1),
                             labels=c("No mpred", "mpred (1m)"))
dgf_filter$mpred_1_3m = factor(dgf_filter$mpred_1_3m,
                               levels=c(0,1),
                               labels=c("No mpred", "mpred (1-3m)"))
dgf_filter$mpred_3m = factor(dgf_filter$mpred_3m,
                             levels=c(0,1),
                             labels=c("No mpred", "mpred (3m)"))
dgf_filter$mpred_3_12m = factor(dgf_filter$mpred_3_12m,
                                levels=c(0,1),
                                labels=c("No mpred", "mpred (3-12m)"))
dgf_filter$mpred_12m = factor(dgf_filter$mpred_12m,
                              levels=c(0,1),
                              labels=c("No mpred", "mpred (12m)"))
dgf_filter$mpred_post12m = factor(dgf_filter$mpred_post12m,
                                  levels=c(0,1),
                                  labels=c("No mpred", "mpred (>12m)"))
dgf_filter$steroid_res_0_1m = factor(dgf_filter$steroid_res_0_1m,
                                     levels=c(0,1),
                                     labels=c("No steroid_res", "steroid_res (0-1m)"))
dgf_filter$steroid_res_1m = factor(dgf_filter$steroid_res_1m,
                                   levels=c(0,1),
                                   labels=c("No steroid_res", "steroid_res (1m)"))
dgf_filter$steroid_res_1_3m = factor(dgf_filter$steroid_res_1_3m,
                                     levels=c(0,1),
                                     labels=c("No steroid_res", "steroid_res (1-3m)"))
dgf_filter$steroid_res_3m = factor(dgf_filter$steroid_res_3m,
                                   levels=c(0,1),
                                   labels=c("No steroid_res", "steroid_res (3m)"))
dgf_filter$steroid_res_3_12m = factor(dgf_filter$steroid_res_3_12m,
                                      levels=c(0,1),
                                      labels=c("No steroid_res", "steroid_res (3-12m)"))
dgf_filter$steroid_res_12m = factor(dgf_filter$steroid_res_12m,
                                    levels=c(0,1),
                                    labels=c("No steroid_res", "steroid_res (12m)"))
dgf_filter$steroid_res_post12m = factor(dgf_filter$steroid_res_post12m,
                                        levels=c(0,1),
                                        labels=c("No steroid_res", "steroid_res (>12m)"))
dgf_filter$bkvan_0_1m = factor(dgf_filter$bkvan_0_1m,
                               levels=c(0,1),
                               labels=c("No bkvan", "bkvan (0-1m)"))
dgf_filter$bkvan_1m = factor(dgf_filter$bk_van_1m,
                             levels=c(0,1),
                             labels=c("No bkvan", "bkvan (1m)"))
dgf_filter$bkvan_1_3m = factor(dgf_filter$bk_van_1_3m,
                               levels=c(0,1),
                               labels=c("No bkvan", "bkvan (1-3m)"))
dgf_filter$bkvan_3m = factor(dgf_filter$bk_van_3m,
                             levels=c(0,1),
                             labels=c("No bkvan", "bkvan (3m)"))
dgf_filter$bkvan_3_12m = factor(dgf_filter$bk_van_3_12m,
                                levels=c(0,1),
                                labels=c("No bkvan", "bkvan (3-12m)"))
dgf_filter$bkvan_12m = factor(dgf_filter$bk_van_12m,
                              levels=c(0,1),
                              labels=c("No bkvan", "bkvan (12m)"))
dgf_filter$bkvan_post12m = factor(dgf_filter$bk_van_post12m,
                                  levels=c(0,1),
                                  labels=c("No bkvan", "bkvan (>12m)"))
dgf_filter$induct_extra = factor(dgf_filter$induct_extra,
                                 levels = c(0,1,2,3,4,5),
                                 labels = c("standard", "ritux", "plex", "eci", "ATG", "bortezomib"))
dgf_filter$cmv_viremia = as.factor(dgf_filter$cmv_viremia)
dgf_filter$cmv_disease = as.factor(dgf_filter$cmv_disease)
dgf_filter$ebv_infection = as.factor(dgf_filter$ebv_infection)
dgf_filter$bkvan  = as.factor(dgf_filter$bkvan)
dgf_filter$bk_viremia  = as.factor(dgf_filter$bk_viremia)
dgf_filter$gasteroenteritis = as.factor(dgf_filter$gasteroenteritis)
dgf_filter$chest_infection  = as.factor(dgf_filter$chest_infection)
dgf_filter$fungal_invasive_orblood  = as.factor(dgf_filter$fungal_invasive_orblood)
dgf_filter$recurrent_uti = as.factor(dgf_filter$recurrent_uti)
dgf_filter$resistant_uti = as.factor(dgf_filter$resistant_uti)
dgf_filter$PTLD  = as.factor(dgf_filter$PTLD)
dgf_filter$skin_ca  = as.factor(dgf_filter$skin_ca)
dgf_filter$other_ca  = as.factor(dgf_filter$other_ca)
dgf_filter$cvd_0_5y  = as.factor(dgf_filter$cvd_0_5y)

dgf_filter$difftime1 = difftime("2020-10-01", dgf_filter$tx_date)/30.4375
dgf_filter$time_to_bpar1 = ifelse(is.na(dgf_filter$time_to_bpar) == TRUE,
                                  dgf_filter$difftime1, dgf_filter$time_to_bpar)
dgf_filter$time_to_subclin1 = ifelse(is.na(dgf_filter$time_to_subclin) == TRUE,
                                     dgf_filter$difftime1, dgf_filter$time_to_subclin)
dgf_filter$time_to_bkvan1 = ifelse(is.na(dgf_filter$time_to_bkvan) == TRUE,
                                   dgf_filter$difftime1, dgf_filter$time_to_bkvan)
dgf_filter$time_to_graft_loss1 = ifelse(is.na(dgf_filter$time_to_graft_loss) == TRUE,
                                        dgf_filter$difftime1, dgf_filter$time_to_graft_loss)
dgf_filter$time_to_death_old1 = ifelse(is.na(dgf_filter$time_to_death_old) == TRUE,
                                       dgf_filter$difftime1, dgf_filter$time_to_death_old)
#

dgf_filter$temp.months = round(as.numeric(difftime(dgf_filter$review_date_J_S, dgf_filter$date_tx,  
                                                   units = "weeks"))/4, digits = 1)
dgf_filter$temp.event = round(as.numeric(difftime(dgf_filter$date_loss, dgf_filter$date_tx,  
                                                  units = "weeks"))/4, digits = 1)

dgf_filter$time.in.study = dgf_filter$temp.months
dgf_filter$time.in.study = ifelse(!is.na(dgf_filter$temp.event), dgf_filter$temp.event, 
                                  dgf_filter$time.in.study)


dgf_filter$graft_age_loss_months = ifelse(dgf_filter$censor_followup == "current", 
                                          dgf_filter$graft_age_loss_months, NA)

dgf_filter$bpar.time = as.numeric(ifelse(is.na(dgf_filter$time_to_bpar), 
                                         dgf_filter$time.in.study, 
                                         dgf_filter$time_to_bpar))
dgf_filter$subclin.time = as.numeric(ifelse(is.na(dgf_filter$time_to_subclin), 
                                            dgf_filter$time.in.study, 
                                            dgf_filter$time_to_subclin))
dgf_filter$graft.loss.noncensored = ifelse(dgf_filter$event_all == "graft_loss_only", 1, 0)
dgf_filter$graft.loss.noncensored = ifelse(is.na(dgf_filter$event_all), 0,
                                           dgf_filter$graft.loss.noncensored)


levels(as.factor(dgf_filter$renal_dx_old))
dgf_filter$renal_dx = factor(dgf_filter$renal_dx,  levels=c(1,2,3,4,5,0),
                             labels=c("T1DM", "T2DM","GN", "HTN", "PKD", "Other"))

GNs = c(dgf_filter$renal_dx_old[grep("alport", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("gn", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("hereditary", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("familial", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("fsgs", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("iga", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("gbm", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("mcgn", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("memranous", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("wegners", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("anca", dgf_filter$renal_dx_old, ignore.case = TRUE)])

pkd = c(dgf_filter$renal_dx_old[grep("pckd", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("pkd", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("polycystic", dgf_filter$renal_dx_old, ignore.case = TRUE)])

T1DM = c(dgf_filter$renal_dx_old[grep("t1dm", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
         dgf_filter$renal_dx_old[grep("type 1", dgf_filter$renal_dx_old, ignore.case = TRUE)])
T1DM = T1DM[! T1DM %in% GNs]
T1DM = T1DM[! T1DM %in% pkd]

T2DM = c(dgf_filter$renal_dx_old[grep("t2dm", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
         dgf_filter$renal_dx_old[grep("type 2", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
         dgf_filter$renal_dx_old[grep("diabetes", dgf_filter$renal_dx_old, ignore.case = TRUE)])
T2DM = T2DM[! T2DM %in% GNs]
T2DM = T2DM[! T2DM %in% pkd]
T2DM = T2DM[! T2DM %in% T1DM]

HTN = c(dgf_filter$renal_dx_old[grep("htn", dgf_filter$renal_dx_old, ignore.case = TRUE)], 
        dgf_filter$renal_dx_old[grep("hypertension", dgf_filter$renal_dx_old, ignore.case = TRUE)])
HTN = HTN[!HTN %in% GNs]
HTN = HTN[!HTN %in% pkd]
HTN = HTN[!HTN %in% T1DM]
HTN = HTN[!HTN %in% T2DM]

dgf_filter = dgf_filter %>% mutate(renal_dx = 
                        case_when(renal_dx_old %in% GNs ~ "GN",
                                  renal_dx_old %in% pkd ~ "PCKD",
                                  renal_dx_old %in% T1DM ~ "T1DM",
                                  renal_dx_old %in% T2DM ~ "T2DM",
                                  renal_dx_old %in% HTN ~ "HTN"))
dgf_filter$renal_dx = ifelse(is.na(dgf_filter$renal_dx), "Other", dgf_filter$renal_dx)

dgf_filter$dgf_hd_days = as.numeric(ifelse(is.na(dgf_filter$dgf_hd_days),
                                           0,  dgf_filter$dgf_hd_days))

# clean up further
dgf_filter$induct_extra1 = ifelse(dgf_filter$induct_extra == 
                                    "standard", "standard", "extraIS")

dgf_filter$induct_extra1 = as.factor(ifelse(is.na(dgf_filter$induct_extra), 
                                            "standard",dgf_filter$induct_extra1))

dgf_filter$HLA_mm = as.numeric(ifelse(is.na(dgf_filter$HLA_mm), 
                                      median(dgf_filter$HLA_mm, na.rm = TRUE), 
                                      dgf_filter$HLA_mm))

dgf_filter$donor_ionotropes = as.factor(ifelse(is.na(dgf_filter$donor_ionotropes),0, 1))

dgf_filter$cit_min = as.numeric(ifelse(is.na(dgf_filter$cit_min), 
                                       median(dgf_filter$cit_min, na.rm = TRUE), 
                                       dgf_filter$cit_min))

dgf_filter$donor_terminal_cr = as.numeric(ifelse(is.na(dgf_filter$donor_terminal_cr), 
                                                 median(dgf_filter$donor_terminal_cr, na.rm = TRUE), 
                                                 dgf_filter$donor_terminal_cr))

dgf_filter$donor_intubated_days = as.numeric(ifelse(is.na(dgf_filter$donor_intubated_days), 
                                                    median(dgf_filter$donor_intubated_days, na.rm = TRUE), 
                                                    dgf_filter$donor_intubated_days))

dgf_filter$donor_age = as.numeric(ifelse(is.na(dgf_filter$donor_age), 
                                         median(dgf_filter$donor_age, na.rm = TRUE), 
                                         dgf_filter$donor_age))

levels(dgf_filter$dsa1_pre) = c(0,1)
dgf_filter$dsa1_pre = as.factor(ifelse(is.na(dgf_filter$dsa1_pre), 0, dgf_filter$dsa1_pre))
dgf_filter$dsa1_pre = as.factor(ifelse(dgf_filter$dsa1_pre == 2, 1, 0))

levels(dgf_filter$dsa2_pre) = c(0,1)
dgf_filter$dsa2_pre = as.factor(ifelse(is.na(dgf_filter$dsa2_pre), 0, dgf_filter$dsa2_pre))
dgf_filter$dsa2_pre = as.factor(ifelse(dgf_filter$dsa2_pre == 2, 1, 0))

dgf_filter$dm_pre = ifelse(is.na(dgf_filter$dm_pre), "Nil", dgf_filter$dm_pre)
dgf_filter$dm_pre = ifelse(dgf_filter$dm_pre ==2, "DM", "Nil")

dgf_filter$htn_pre = ifelse(is.na(dgf_filter$htn_pre), "Nil", dgf_filter$htn_pre)
dgf_filter$htn_pre = ifelse(dgf_filter$htn_pre ==2, "HTN", "Nil")

dgf_filter$ihd_pre = ifelse(is.na(dgf_filter$ihd_pre), "Nil", dgf_filter$ihd_pre)
dgf_filter$ihd_pre = ifelse(dgf_filter$ihd_pre ==2, "IHD", "Nil")

dgf_filter$chol_pre = ifelse(is.na(dgf_filter$chol_pre), "Nil", dgf_filter$chol_pre)
dgf_filter$chol_pre = ifelse(dgf_filter$chol_pre ==2, "Chol", "Nil")

dgf_filter$donor_ionotropes = ifelse(is.na(dgf_filter$donor_ionotropes), 0, dgf_filter$donor_ionotropes)
dgf_filter$donor_ionotropes = as.factor(ifelse(dgf_filter$donor_ionotropes ==2, 1, 0))

dgf_filter$graft_number = ifelse(is.na(dgf_filter$graft_number), "1st Graft", dgf_filter$graft_number)
dgf_filter$graft_number = as.factor(ifelse(dgf_filter$graft_number ==2, "Regraft", "1st Graft"))



saveRDS(dgf_filter, "data/annotatedDB.RDS")
gc()

# Section 2: sequencing data pre-processing

jen1 = dgf_filter

# CSL runs
Counts = readRDS("data/base/Biopsy/Batch 1/CSL1_allStrandedCounts.RDS")
Counts.2 = readRDS("data/base/Biopsy/Batch 2/CSL2_allStrandedCounts.RDS")
Biopsy.Counts.CSL = cbind(Counts, Counts.2)

Samples = readRDS("data/base/Biopsy/Batch 1/CSL1_allStrandedSamples.RDS")
Samples.2 = readRDS("data/base/Biopsy/Batch 2/CSL2_allStrandedSamples.RDS")
Biopsy.Samples.CSL = rbind(Samples, Samples.2)
Biopsy.Samples.CSL$time = as.factor(Biopsy.Samples.CSL$time)

levels(Biopsy.Samples.CSL$time) = c("0 Month", "1 Month", "3 Month")
Biopsy.Samples.CSL$batch = as.factor(Biopsy.Samples.CSL$batch)

levels(Biopsy.Samples.CSL$batch) = c("CSL Batch 1", "CSL Batch 2")
Biopsy.Samples.CSL = Biopsy.Samples.CSL %>%  mutate(Sample.Type = "Renal.Biopsy")
Biopsy.Samples.CSL$Sample.Type = as.factor(Biopsy.Samples.CSL$Sample.Type)

# AGRF run
Biopsy.Counts.AGRF = readRDS("data/base/AGRF Quality Scores/AGRF2020_COMPLETE_allStrandedCounts.RDS")
Biopsy.Samples.AGRF = readRDS("data/base/AGRF Quality Scores/AGRF2020_COMPLETE_allStrandedSamples.RDS")


Biopsy.Samples.AGRF$Time.point = as.factor(Biopsy.Samples.AGRF$Time.point)
Biopsy.Samples.AGRF$time = Biopsy.Samples.AGRF$Time.point

Biopsy.Samples.AGRF$folder = as.factor(Biopsy.Samples.AGRF$folder)
levels(Biopsy.Samples.AGRF$folder) = c("AGRF Batch 1", "AGRF Batch 2",
                                       "AGRF Batch 3", "AGRF Batch 4",
                                       "AGRF Batch 5", "AGRF Batch 6",
                                       "AGRF Batch 7", "AGRF Batch 8",
                                       "AGRF Batch 9", "AGRF Batch 10")
Biopsy.Samples.AGRF$batch = Biopsy.Samples.AGRF$folder

Biopsy.Samples.AGRF = Biopsy.Samples.AGRF[,c("ID", "time", "batch")]
rownames(Biopsy.Samples.AGRF) = sub("s_", "", rownames(Biopsy.Samples.AGRF))

blood.0 = which(rownames(Biopsy.Samples.AGRF) %in% jen1$RNABlood_0M_ID)
blood.3 = which(rownames(Biopsy.Samples.AGRF) %in% jen1$RNABlood_3M_ID)
blood.sample = c(blood.0, blood.3)

Biopsy.Samples.AGRF = Biopsy.Samples.AGRF %>% mutate(Sample.Type = "Renal.Biopsy")
Biopsy.Samples.AGRF$Sample.Type[blood.sample] = "Blood"
Biopsy.Samples.AGRF$Sample.Type = as.factor(Biopsy.Samples.AGRF$Sample.Type)
rownames(Biopsy.Samples.AGRF) = colnames(Biopsy.Counts.AGRF)

# Combine all available sequencing
Biopsy.Samples.AGRF = Biopsy.Samples.AGRF[,c(2:4)]
colnames(Biopsy.Samples.AGRF) = colnames(Biopsy.Samples.CSL)
Counts = cbind(Biopsy.Counts.CSL, Biopsy.Counts.AGRF)
Samples = rbind(Biopsy.Samples.CSL, Biopsy.Samples.AGRF)
Samples$row.n = rownames(Samples)


# Clean up of label and uncertain samples - remove
# Make sure the samples are found in the documentation
rownames(Samples)[grep("J.H76", rownames(Samples))] =  c("J76_3", "J76", "J76_1")
colnames(Counts)[grep("J.H76", colnames(Counts))] =  c("J76_3", "J76", "J76_1")

Samples.Removed = c("s_131", "s_66", "s_134", "s_165",
                    "s_244", "s_90", "s_342", "s_118", "s_341")

Counts = Counts[,-which(colnames(Counts) %in% Samples.Removed)]
Samples = Samples[-which(rownames(Samples) %in% Samples.Removed),]
rownames(Samples) = colnames(Counts)

rownames(Samples)[grep("s_", rownames(Samples))] =
  str_remove(rownames(Samples)[grep("s_", rownames(Samples))], "s_")

colnames(Counts)[grep("s_", colnames(Counts))] =
  str_remove(colnames(Counts)[grep("s_", colnames(Counts))], "s_")
...
biopsy.sample.id = c(jen1$RNABiopsy_0M_ID, jen1$RNABiopsy_1M_ID, jen1$RNABiopsy_3M_ID)
biopsy.sample.id = biopsy.sample.id[!is.na(biopsy.sample.id)]

Samples = Samples[rownames(Samples) %in% biopsy.sample.id,]
Counts = Counts[,colnames(Counts) %in% biopsy.sample.id]

# Join the sample and clinical sheets
jen1 = data.frame(jen1)
rownames(jen1) = jen1[,1]

# 0m kidney biopsy combination
biopsy0m = Samples %>% filter(Sample.Type == "Renal.Biopsy") %>% filter(time == "0 Month")
biopsy0m = cbind(biopsy0m, jen1[match(rownames(biopsy0m), jen1$RNABiopsy_0M_ID),])
counts0m = Counts[,colnames(Counts) %in% rownames(biopsy0m)]
counts0m = counts0m[5:nrow(counts0m),]

# 1m kidney biopsy combination
biopsy1m = Samples %>% filter(Sample.Type == "Renal.Biopsy") %>% filter(time == "1 Month")
biopsy1m = cbind(biopsy1m, jen1[match(rownames(biopsy1m), jen1$RNABiopsy_1M_ID),])
counts1m = Counts[,colnames(Counts) %in% rownames(biopsy1m)]
counts1m = counts1m[5:nrow(counts1m),]

# 3m kidney biopsy combination
biopsy3m = Samples %>% filter(Sample.Type == "Renal.Biopsy") %>% filter(time == "3 Month")
biopsy3m = cbind(biopsy3m, jen1[match(rownames(biopsy3m), jen1$RNABiopsy_3M_ID),])
counts3m = Counts[,colnames(Counts) %in% rownames(biopsy3m)]
counts3m = counts3m[5:nrow(counts3m),]

# 0m blood combination
blood0m = Samples.all %>% filter(Sample.Type == "Blood") %>% filter(time == "0 Month")
blood0m = cbind(blood0m, jen1[match(rownames(blood0m), jen1$RNABlood_0M_ID),])
bloodcounts0m = Counts.all[,colnames(Counts.all) %in% rownames(blood0m)]
bloodcounts0m = bloodcounts0m[5:nrow(bloodcounts0m),]

# 3m blood combination
blood3m = Samples.all %>% filter(Sample.Type == "Blood") %>% filter(time == "3 Month")
blood3m = cbind(blood3m, jen1[match(rownames(blood3m), jen1$RNABlood_3M_ID),])
bloodcounts3m = Counts.all[,colnames(Counts.all) %in% rownames(blood3m)]
bloodcounts3m = bloodcounts3m[5:nrow(bloodcounts3m),]

# Conversions between symbol, ensembl and entrezid
conversions = rownames(Counts[5:nrow(Counts),])
Symbol.1 <- mapIds(org.Hs.eg.db, keys = conversions, keytype = "ENSEMBL",
                   column = "SYMBOL", multiVals = "first")
Symbol.2 <- mapIds(org.Hs.eg.db, keys = conversions, keytype = "ENSEMBL",
                   column = "ENTREZID", multiVals = "first")
conversions = cbind(conversions, Symbol.1, Symbol.2)
conversion = data.frame(conversions)
...
auscad.sorted = new("auscad.sorted",
                    ...
                    samples.biopsy.0m = biopsy0m,
                    counts.biopsy.0m = counts0m,
                    samples.biopsy.1m = biopsy1m,
                    counts.biopsy.1m = counts1m,
                    samples.biopsy.3m = biopsy3m,
                    counts.biopsy.3m = counts3m,
                    samples.blood.0m = blood0m,
                    counts.blood.0m = bloodcounts0m,
                    samples.blood.3m = blood3m,
                    counts.blood.3m = bloodcounts3m)

saveRDS(auscad.sorted, "data/auscad.sorted.RDS")


saveRDS(Samples.all, "data/samples.all.RDS")
saveRDS(Counts.all, "data/counts.all.RDS")
saveRDS(conversion, "data/conversion.RDS")
saveRDS(biopsy0m, "data/biopsy0m.RDS")
saveRDS(counts0m, "data/counts0m.RDS")
saveRDS(biopsy1m, "data/biopsy1m.RDS")
saveRDS(counts1m, "data/counts1m.RDS")
saveRDS(biopsy3m, "data/biopsy3m.RDS")
saveRDS(counts3m, "data/counts3m.RDS")
saveRDS(blood0m, "data/blood0m.RDS")
saveRDS(bloodcounts0m, "data/bloodcounts0m.RDS")
saveRDS(blood3m, "data/blood3m.RDS")
saveRDS(bloodcounts3m, "data/bloodcounts3m.RDS")
...
# Make into SingleCellExperiment
sce = SingleCellExperiment(list(counts=gse.a))
libsizes = colSums(gse.a)
size.factors = libsizes/mean(libsizes)
logcounts(sce) = log2(t(t(gse.a)/size.factors) + 1)
